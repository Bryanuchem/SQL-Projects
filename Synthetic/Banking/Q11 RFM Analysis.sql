WITH RFM_Values AS (
    SELECT
        accountid,
         DATEDIFF(max(date), Min(date)) AS Recency, -- This calculates the span of orders, not true recency
       -- DATEDIFF(CURRENT_DATE(), MAX(date)) AS Recency, -- Calculate Recency from last order to today
...
Combined_RFM_Scores AS ( -- Renamed 'cte' for clarity
    ...
    ON rs.accountid = fs.accountid
    JOIN Monetary_Scored ms 
    ON rs.accountid = ms.accountid
),

Customer_RFM_Segments AS ( -- Renamed 'cte_2' for clarity
    SELECT
        accountid, -- Keep customer_id for potential joins if needed later
        crs.Recency_Score,
        crs.Frequency_Score,
        crs.Monetary_Score,
        CONCAT(
